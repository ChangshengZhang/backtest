...
class CreepyDataFromWindClass():
	...
	def get_latest_date_in_fold(self,data_path):
		...
		for data_day in data_day_list:
			if float(data_day)>max_day:
				max_day = float(data_day)
		return str(data_day)
	...
	def creepy_data_from_wind_to_local(self):
		...
		for ii in range(len(self.user_stock_name_list)):
			...
			for data_type in self.data_type_list :
				...
				delta_day = (datetime.now()-datetime(int(latest_date[0:4]),int(latest_date[4:6]),latest_date[6:10])).days
				...
				if data_type =="daily":
					...
					for jj in range(len(date_60_min_index)-1):
						file_path = data_path+"\\"+stock_60_min_data[0][date_60_min_index[jj]][0][0:10]+".txt"
						WriteToFile.write_list_to_file(file_path,date_60_min_index[0][date_60_min_index[jj]:date_60_min_index[jj+1]])
					...
					for jj in range(len(date_30_min_index)-1):
						file_path = data_path +"\\"+stock_30_min_data[0][date_30_min_index[jj]][0][0:10]+".txt"
						WriteToFile.write_list_to_file(file_path,date_30_min_index[0][date_30_min_index[jj]:date_30_min_index[jj+1]])
					...
					for jj in range(len(date_15_min_index)-1):
						file_path = data_path +"\\"+stock_15_min_data[0][date_15_min_index[jj]][0][0:10]+".txt"
						WriteToFile.write_list_to_file(file_path,date_15_min_index[0][date_15_min_index[jj]:date_15_min_index[jj+1]])
					...
					for jj in range(len(date_5_min_index)-1):
						file_path = data_path +"\\"+stock_15_min_data[0][date_5_min_index[jj]][0][0:10]+".txt"
						WriteToFile.write_list_to_file(file_path,date_5_min_index[0][date_15_min_index[jj]:date_5_min_index[jj+1]])
